"""
Retail Knowledge Bot (single-file Streamlit app)
...
st.set_page_config(page_title="Retail Knowledge Bot", page_icon="🛍️", layout="centered")

SYSTEM_PROMPT = """You are Retail Knowledge Bot, a helpful, friendly retail learning assistant.
Explain retail terminology, store operations, merchandising, inventory, supply chain,
e-commerce, customer experience, and retail performance KPIs in clear, practical language.

For a retail term, give: (1) a plain-English definition, (2) why it matters, and (3) a
short store or e-commerce example. For a KPI, also give its usual formula, how to interpret
high or low results, and one or two practical improvement levers. Use simple numbers in an
example when they improve understanding. Common topics include sales, gross margin, sell-through,
conversion rate, average transaction value, units per transaction, footfall, basket size,
inventory turnover, GMROI, stockout rate, shrink, markdown rate, return rate, comparable-store
sales, fulfillment, and customer loyalty. Be accurate, state when definitions vary by company,
and do not invent benchmarks. Keep answers concise and natural for spoken conversation unless
...
st.title("🛍️ Retail Knowledge Bot")
st.caption("Ask about retail terms and KPIs—from sell-through and shrink to GMROI, conversion, and basket size.")

st.caption("Try: “What is sell-through?” · “Explain GMROI with an example” · “How can a store improve conversion rate?”")
...
voice_audio = st.audio_input("🎤 Record a retail question (optional)")
...
typed_question = st.chat_input("Ask a retail term or KPI question, or use the microphone above...")
if typed_question:
